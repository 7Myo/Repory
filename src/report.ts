...
const paint = (value: string, code: string, enabled: boolean) =>
  enabled ? `${code}${value}${ansi.reset}` : value;
const bar = (n: number | null, color: boolean) => {
...
export function progress(message: string, color: boolean): void {
  process.stderr.write(`${paint("◆", ansi.cyan, color)} ${message}\n`);
}
export function welcome(color: boolean): string {
  return [
    "",
...
export function markdown(a: Analysis) {
  const s = a.scores;
  return `# Repory — ${a.repository.name}\n\n> Understand your codebase\n\n## Scores\n\n| Metric | Score |\n|---|---:|\n| Architecture | ${s.architecture ?? "N/A"} |\n| Complexity | ${s.complexity ?? "N/A"} |\n| Maintainability | ${s.maintainability ?? "N/A"} |\n| Documentation | ${s.documentation ?? "N/A"} |\n| Security indicators | ${s.security ?? "N/A"} |\n| **Overall** | **${s.overall ?? "N/A"}** |\n\n## Project\n\n- Branch: ${a.repository.branch}\n- Files: ${a.project.files}\n- Lines: ${a.project.lines}\n- Commits: ${a.project.commits}\n- Contributors: ${a.project.contributors}\n- Languages: ${
    Object.entries(a.project.languages)
      .map(([k, v]) => `${k} (${v})`)
      .join(", ") || "N/A"
...
export function terminal(a: Analysis, color = true) {
  const s = a.scores;
  const line = paint("─".repeat(68), ansi.dim, color);
  const title = (value: string) =>
    `${paint("┌", ansi.blue, color)} ${paint(value, ansi.bold + ansi.cyan, color)}`;
