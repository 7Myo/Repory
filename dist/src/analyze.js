...
import { history, branch } from "./git.js";
...
function files(root, skip) {
    const out = [];
    const walk = (d) => {
        for (const e of readdirSync(d, { withFileTypes: true })) {
            if (e.name.startsWith(".") && e.name !== ".env")
                continue;
            if (ignored.has(e.name) || skip.includes(e.name))
                continue;
            ...
            if (e.isDirectory())
                ...
                out.push({
                    ...
                    deletions: 0,
                    contributors: 0,
                    complexity: (text.match(/\b(if|for|while|case|catch|\?\?)\b/g) || []).length +
                        1,
                });
            }
        }
...
function detectArchitecture(root, fs) {
    ...
    if (names.has("package.json")) {
        try {
            ...
            if (/next/.test(all))
                framework = "Next.js";
            else if (/react/.test(all))
                framework = "React";
            else if (/express|fastify|koa/.test(all))
...
export function analyze(root, source, skip = []) {
    ...
    const languages = {};
    fs.forEach((f) => (languages[f.language] = (languages[f.language] || 0) + f.lines));
    const contributorsMap = new Map();
    commits.forEach((c) => contributorsMap.set(c.author, (contributorsMap.get(c.author) || 0) + 1));
    ...
    const maintain = score(complexityScore * 0.4 +
        docScore * 0.25 +
        (commits.length ? 25 : 0) +
    ...
    for (const c of commits) {
        ...
        if (!years.has(y))
            years.set(y, []);
        if (c.files > 10)
            years.get(y).push(c.subject || "Large change");
    }
    return {
        repository: {
