...
export function git(cwd, args) {
    ...
    if (r.error)
        throw new Error("Git is required to analyze repository history. Install Git and try again.");
    if (r.status !== 0)
        throw new Error((r.stderr || "Git command failed").trim());
    return r.stdout;
}
export function prepare(input, options) {
    const remote = /^https?:\/\/github\.com\/([^/]+)\/([^/#?]+)\/?$/.exec(input) ||
    ...
    if (remote) {
        ...
        try {
            ...
            throw new Error("Repository could not be accessed. It may be private or unavailable.");
        }
        return {
            path: dir,
            source: input,
...
export function branch(path) {
    try {
        return (git(path, ["branch", "--show-current"]).trim() ||
